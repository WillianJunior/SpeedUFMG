...
| Quotas | Não | Por user | Por projeto (ainda não...) |
...
Operações de I/O pode degradar severamente o desempenho de suas aplicações (usem htop mostrando o PSI para ver o dano). Para que seus jobs rodem mais rápido tentem seguir as seguintes recomendações:
 1. Usem /scratch sempre que possível para arquivos muito usados. Baixem esses dados de /snfs2 ou /home para /scratch ao início do job. Caso precisem dos resultados em /scratch, não esqueça de copiar os arquivos de volta para /snfs2.
 ...
 3. O que não fica em /home vai para /snfs2.
 4. Não coloquem arquivos diretamente na raiz do storage (e.g., um arquivo `/snfs2/meu_arquivo.txt`) pois esses podem ser **apagados** por qualquer usuário. Crie antes seu diretório (e.g., `/snfs2/username/meu_arquivo.txt`)
...
O `/snfs2` foi implementado via BeeGFS nas medusas. Cada medusa disponibiliza 6 TB para o mount global. Embora suportado, **não foi configurado stripping (RAID0)**. Também não é suportado mirroring. Como alguns nós falham com frequência não insignificativa, e não temos mirroring por nós, a falha de um nó com stripping significa perda de dados. Para evitar isso, arquivos são mantidos em apenas 1 nó (balanceado pelo BeeGFS). Outro resultado disso: tempos de acesso podem ser muito rápidos caso o arquivo esteja no mesmo nó (basicamente bare metal)! Como a rede do lab é sobrecarregada, isso ajuda a todos.

### Falhas

Em caso de falhas simples de nós (sem rede) acessos são congelados. I.e., abrir um arquivo trava o processo em I/O. A restauração do nó inacessível resolve automaticamente o problema.

Em caso de falhas completas (nó sem funcionar ou disco morreu) dados serão perdidos. Porém, como o target de storage está apenas offline para o BeeGFS, acessos até mesmo via `ls` congelam se um arquivo *corrompido* estiver nesse dir. O que fazer: remover o target que caiu, assim pode-se realizar `ls` nos dirs. Arquivos que não estiverem no nó morto podem ser acessados normalmente. Para mais detalhes consultar [SONIC2/install_scripts_s2/medusa-beegfs.sh](aqui como resolver falhas de nós).
